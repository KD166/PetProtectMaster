...
//try cach para lidar com possíveis exceções
  ...
  public void adicionarAnimal(Animal animal, String imagemPath) {
        try (Connection conn = Database.getConnection()) {
            String sql = "INSERT INTO public.animal (localizacao, nome, estado, imagem, raca, especie) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public void atualizarAnimal(Animal animal, String imagemPath) {
        try (Connection conn = Database.getConnection()) {
            String sql = "UPDATE public.animal SET localizacao=?, nome=?, estado=?, imagem=?, raca=?, especie=? WHERE id=?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
